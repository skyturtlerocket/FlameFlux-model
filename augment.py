...
def collectData(fireName, days):
    ...
    print('landsat4.tif shape: ', landsat4.shape)
    landsat3 = util.openImg('data/'+ fireName + '/band_3.tif')
    print('landsat3.tif shape: ', landsat3.shape)
    ...
    # save orig as npy
    # np.save('data/'+fireName+'/dem.npy', dem)
    # np.save('data/'+fireName+'/aspect.npy', aspect)
    # np.save('data/'+fireName+'/band_4.npy', landsat4)
    # np.save('data/'+fireName+'/band_3.npy', landsat3)
    # np.save('data/'+fireName+'/band_2.npy', landsat2)
    # np.save('data/'+fireName+'/band_5.npy', landsat5)
    # np.save('data/'+fireName+'/ndvi.npy', ndvi)
    # np.save('data/'+fireName+'/slope.npy', slope)
    ...
    for day in days:
        ...
        np.save('data/'+fireName+'/perims/'+day+'.npy', days_arr[-1])
...
def doMore(toaugment, fire, days, f_tuple, perim_array, deg):
    infinity = Decimal('Infinity')
    oidg = image.ourImageDataGenerator(
            rotation_range=deg, #40, 80, 120, 160, 200, 240, 300, 360, 280, 330
...
def saveFiles(augmented, int_index, perim_array, days):
    ...
    for n, perim in enumerate(cv2.split(othersperims)):
        util.saveImg(folder + '/perims/' + days[n], perim )
    util.saveImg(folder + '/dem', dem)
    util.saveImg(folder + '/ndvi', ndvi)
    util.saveImg(folder + '/aspect', aspect)
    util.saveImg(folder + '/slope', slope)
    np.savetxt('landsatrightbeforesavetif.csv', landsat[:,:,2], delimiter=',')
    util.saveImg(folder + '/band_4', landsat[:,:,0])
    util.saveImg(folder + '/band_3', landsat[:,:,1])
    util.saveImg(folder + '/band_2', landsat[:,:,2])
    util.saveImg(folder + '/band_5', landsat[:,:,3])
...
fires = ['riceRidge', 'coldSprings', 'beaverCreek', 'haydenPass', 'junkins', 'peekaboo', 'redDirt', 'gutzler', 'ecklund', 'redDirt2']
rrdays = ['0731', '0801', '0802', '0803'] #, '0809', '0810', '0811', '0812', '0813', '0814', '0815'
...
spin_amt = 20
while deg < 360:
    print("SPINNING: ", spin_amt)
    for fire in fires:
        if fire == fires[0]:
            toaugment, fire_tuple, day_arr = collectData(fire, rrdays)
            doMore(toaugment, fire, rrdays, fire_tuple, day_arr, deg)
        if fire == fires[1]:
            toaugment, fire_tuple, day_arr = collectData(fire, csdays)
            doMore(toaugment, fire, csdays, fire_tuple, day_arr, deg)
        if fire == fires[3]:
            toaugment, fire_tuple, day_arr = collectData(fire, hpdays)
            doMore(toaugment, fire, hpdays, fire_tuple, day_arr, deg)
        if fire == fires[4]:
            toaugment, fire_tuple, day_arr = collectData(fire, jdays)
            doMore(toaugment, fire, jdays, fire_tuple, day_arr, deg)
        if fire == fires[5]:
            toaugment, fire_tuple, day_arr = collectData(fire, peekdays)
            doMore(toaugment, fire, peekdays, fire_tuple, day_arr, deg)
        # if fire == fires[6]:
        ...
        if fire == fires[6]:
            ...
            doMore(toaugment, fire, rddays, fire_tuple, day_arr, deg)
        if fire == fires[7]:
            toaugment, fire_tuple, day_arr = collectData(fire, gdays)
            doMore(toaugment, fire, gdays, fire_tuple, day_arr, deg)
        if fire == fires[8]:
            toaugment, fire_tuple, day_arr = collectData(fire, eckdays)
            doMore(toaugment, fire, eckdays, fire_tuple, day_arr, deg)
        if fire == fires[9]:
            toaugment, fire_tuple, day_arr = collectData(fire, rd2days)
            doMore(toaugment, fire, rd2days, fire_tuple, day_arr, deg)
    deg+=spin_amt
